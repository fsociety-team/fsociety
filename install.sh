...
#
# --- Fsociety Installer ---
# Bash Script for install Fsociety
# Must run as root to install
...
if [ "$PREFIX" = "/data/data/com.termux/files/usr" ]; then
    ...
    pkg install -y python3 python3-pip git
...
if [ -d "$ETC_DIR/fsociety" ]; then
    echo "$DIR_FOUND_TEXT"
    if [ "$TERMUX" = true ]; then
        rm -rf "$ETC_DIR/fsociety"
    else
        sudo rm -rf "$ETC_DIR/fsociety"
    fi
fi
